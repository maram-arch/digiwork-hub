...
class UserModel
{
	...
	{
		$stmt = $this->db->query('SELECT id_user, email, role, tel, is_verified FROM user ORDER BY id_user DESC');
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	}

	public function getById(int $id): ?array
	{
		$stmt = $this->db->prepare('SELECT id_user, email, mdp, role, tel, is_verified FROM user WHERE id_user = :id');
		$stmt->execute(['id' => $id]);
	...
	{
		$stmt = $this->db->prepare('SELECT id_user, email, mdp, role, tel, is_verified FROM user WHERE email = :email LIMIT 1');
	...
	{
		$stmt = $this->db->prepare('UPDATE user SET is_verified = 1 WHERE id_user = :id');
		return $stmt->execute(['id' => $userId]);
	}

	public function isVerified(int $userId): bool
	{
		$stmt = $this->db->prepare('SELECT is_verified FROM user WHERE id_user = :id LIMIT 1');
		$stmt->execute(['id' => $userId]);
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		return $row && (int) $row['is_verified'] === 1;
	}
}
