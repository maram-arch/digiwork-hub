...
public function __construct() {
    ...
    public function updatePublication($id, $titre, $contenu, $categorie = 'general', $image = null, $is_event = 0, $event_date = null, $event_lieu = null) {
        if ($image !== null) {
            $stmt = $this->pdo->prepare(
                "UPDATE forums SET titre=?, contenu=?, categorie=?, image=?, is_event=?, event_date=?, event_lieu=?
                 WHERE id_publication=?"
            );
            return $stmt->execute([$titre, $contenu, $categorie, $image, $is_event, $event_date, $event_lieu, $id]);
        } else {
            $stmt = $this->pdo->prepare(
                "UPDATE forums SET titre=?, contenu=?, categorie=?, is_event=?, event_date=?, event_lieu=?
                 WHERE id_publication=?"
            );
    ...
    public function toggleLike($id_publication, $id_user) {
        $stmt = $this->pdo->prepare(
            "SELECT id FROM publication_likes WHERE id_publication=? AND id_user=?"
        );
        $stmt->execute([$id_publication, $id_user]);
        $existing = $stmt->fetch();

        if ($existing) {
            $this->pdo->prepare("DELETE FROM publication_likes WHERE id_publication=? AND id_user=?")
                       ->execute([$id_publication, $id_user]);
            $this->pdo->prepare("UPDATE forums SET nb_likes = GREATEST(nb_likes-1, 0) WHERE id_publication=?")
                       ->execute([$id_publication]);
            return ['action' => 'unliked'];
        } else {
            $this->pdo->prepare("INSERT INTO publication_likes (id_publication, id_user) VALUES (?,?)")
                       ->execute([$id_publication, $id_user]);
            $this->pdo->prepare("UPDATE forums SET nb_likes = nb_likes+1 WHERE id_publication=?")
                       ->execute([$id_publication]);
            return ['action' => 'liked'];
        }
    }
    ...
    public function uploadImage($file) {
        ...
        if (!in_array($file['type'], $allowedTypes)) {
            return ['error' => 'Type de fichier non autorisé (jpg, png, gif, webp uniquement).'];
        }
        if ($file['size'] > $maxSize) {
            return ['error' => 'Image trop lourde (max 3MB).'];
        }
    ...
    public function isOwner($id_publication, $id_user) {
        $stmt = $this->pdo->prepare(
            "SELECT id_publication FROM forums WHERE id_publication=? AND id_user=?"
        );
        $stmt->execute([$id_publication, $id_user]);
        return (bool)$stmt->fetch();
    }
