<?php
// view/backoffice/addPublication.php
// Formulaire d'ajout de publication (backoffice) – version compatible
...
session_start();
$id_user = $_SESSION['id_user'] ?? 1; // fallback
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        ...
        // Instanciation avec ordre POSITIONNEL (comme dans le modèle)
        $publication = new Publication(
            null,                              // id_publication
            $titre,                            // titre
            $contenu,                          // contenu
            ($date_programmee ? $date_programmee : date('Y-m-d H:i:s')), // date_publication
            $id_user,                          // id_user
            $categorie,                        // categorie
            $image,                            // image
            'active',                          // statut
        ...
        $successMessage = "Publication ajoutée avec succès !";
        // Redirection après succès
        header("Location: index.php?status=success&msg=" . urlencode($successMessage));
        exit;
    } else {
...
<head>
    ...
    <title>Ajouter une publication - Backoffice</title>
    <link rel="stylesheet" href="assets/css/bootstrap.css">
    <link rel="stylesheet" href="assets/css/app.css">
    <style>
        .form-container { max-width: 800px; margin: 40px auto; background: #fff; padding: 30px; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); }
...
<div class="container">
    <div class="form-container">
        <h2 class="mb-4">➕ Ajouter une publication</h2>
        ...
        <form method="POST" action="" enctype="multipart/form-data" onsubmit="return validateAddPublication()">
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="date_programmee" class="form-label">Programmer la publication (optionnel)</label>
...
<script>
    ...
    function validateAddPublication() {
        let isValid = true;
