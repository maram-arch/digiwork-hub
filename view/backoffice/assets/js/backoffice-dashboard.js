(function() {
    ...
    function initializeDashboard() {
        ...
        const sortSelect = document.getElementById('sortSelectDashboard');
        if (sortSelect) {
            sortSelect.addEventListener('change', performSort);
        }

        // Update user count display
        const rows = document.querySelectorAll('.user-row-dashboard');
        document.getElementById('dashboardUserCount').textContent = '(' + rows.length + ')';
    }
    ...
    function performSearch() {
        ...
        if (searchTerm === '') {
            document.getElementById('dashboardUserCount').textContent = '(' + allStats.total + ')';
        } else {
            document.getElementById('dashboardUserCount').textContent = '(' + visibleCount + ' filtré)';
        }
        ...
        if (visibleCount === 0 && searchTerm !== '') {
            ...
            noResultsRow.innerHTML = '<td colspan="4" class="text-center text-muted">Aucun utilisateur trouvé avec ces critères</td>';
    ...
    function performSort() {
        ...
        const rows = Array.from(document.querySelectorAll('.user-row-dashboard'));
        const tbody = document.getElementById('dashboardUsersTableBody');
