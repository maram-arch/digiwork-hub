...
if (isset($_GET['logout']) && $_GET['logout'] == 1) {
    try {
        (new UserController())->logoutUser((int) $loggedInUser['id_user']);
    } catch (Throwable $e) {
    }
    header('Location: login.php');
    exit;
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        } elseif ($action === 'manual_verify') {
            $id = (int) ($_POST['user_id'] ?? 0);
            $result = $controller->manualVerify($id);
        } else {
                ...
                <div class="card-body">
                    <form id="backofficeUserForm" method="post" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="password" name="password" id="boPassword" value="" placeholder="10+ car., maj/min, chiffre, symbole" autocomplete="new-password">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($user['is_verified'])): ?>
                                        <span style="color:#27ae60;font-weight:600;">&#10003; Verifie</span>
                                    <?php else: ?>
                                        <span style="color:#c0392b;">&#10007; Non verifie</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    ...
                                    <?php if (empty($user['is_verified'])): ?>
                                    <form method="post" style="display:inline-block;" onsubmit="return confirm('Verifier manuellement ce compte ?');">
                                        <input type="hidden" name="action" value="manual_verify">
                                        <input type="hidden" name="user_id" value="<?= (int) $user['id_user'] ?>">
                                        <button class="btn btn-sm btn-warning" type="submit">Verifier manuellement</button>
                                    </form>
                                    <?php endif; ?>
                                </td>
                            </tr>
