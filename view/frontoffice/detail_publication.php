...
// 👇 Utilisateur par défaut pour les anonymes
...
Publication::incrementVues($id_pub);
...
// Récupération arborescente des commentaires
...
if (isset($_SESSION['id_user'])) {
    ...
    foreach ($allComments as $c) {
        ...
        if ($stmt->fetchColumn()) $likedCommentsIds[] = $c['id_commentaire'];
    }
}

// Fonction récursive pour l'affichage des commentaires
function renderCommentTree($comments, $likedIds, $level = 0) {
    ...
    foreach ($comments as $c) {
        ...
        $html .= '<div><strong>' . htmlspecialchars($c['prenom'] . ' ' . $c['nom']) . '</strong>';
        $html .= ' <small class="text-muted">le ' . htmlspecialchars($c['date_commentaire']) . '</small></div>';
        $html .= '</div>';
        ...
        $html .= '<button class="btn-repondre" data-id="' . $c['id_commentaire'] . '">💬 Répondre</button>';
        $html .= '</div>';
        ...
        $html .= '<button type="button" class="btn btn-sm btn-secondary annuler-reponse">Annuler</button></div>';
        $html .= '</form></div>';
        if (!empty($c['reponses'])) {
            $html .= '<div class="reponses mt-3">';
            $html .= renderCommentTree($c['reponses'], $likedIds, $level + 1);
            $html .= '</div>';
        }
        $html .= '</div>';
    }
...
<head>
    ...
    <style>
        ...
        body.dark-mode .pub-card, body.dark-mode .comment-card, body.dark-mode .navbar, body.dark-mode .card-pub { background: #2a2a3a; color: #eee; border-color: #444; }
        body.dark-mode .like-btn, body.dark-mode .favori-btn { color: #ccc; }
        body.dark-mode .like-btn.liked, body.dark-mode .favori-btn.active { color: #f1c40f; }
...
<div class="container py-4">
    <?php if (isset($_GET['status'], $_GET['msg'])): ?>
        <div class="alert alert-<?= $_GET['status'] === 'success' ? 'success' : 'danger' ?>"><?= htmlspecialchars($_GET['msg']) ?></div>
    <?php endif; ?>
    ...
    <div class="card-pub">
        <div class="d-flex justify-content-between">
            ...
            <?php if (isset($_SESSION['id_user']) && $_SESSION['id_user'] == $pub['id_user']): ?>
                <div><a href="editPublication.php?id=<?= $id_pub ?>" class="btn btn-sm btn-warning">✏️ Modifier</a></div>
            <?php endif; ?>
        </div>
        <h1 class="mt-3"><?= htmlspecialchars($pub['titre']) ?></h1>
        <p class="text-muted">Par <strong><?= htmlspecialchars($pub['prenom'] . ' ' . $pub['nom']) ?></strong> – le <?= date('d/m/Y H:i', strtotime($pub['date_publication'])) ?></p>
        <?php if (!empty($pub['image'])): ?>
            <img src="../../<?= htmlspecialchars($pub['image']) ?>" class="img-fluid rounded mb-3" style="max-height:300px;">
        <?php endif; ?>
        <div class="content mb-4"><?= nl2br(htmlspecialchars($pub['contenu'])) ?></div>
        <div class="d-flex gap-3 align-items-center">
            <button class="like-btn <?= $hasLiked ? 'liked' : '' ?>" data-id="<?= $id_pub ?>">❤️ <span class="like-count"><?= (int)$pub['nb_likes'] ?></span></button>
            <button class="favori-btn <?= $isFavori ? 'active' : '' ?>" data-id="<?= $id_pub ?>">⭐ Favori</button>
            <span>👁️ <?= (int)$pub['nb_vues'] ?> vues</span>
            <a href="export_pdf.php?id=<?= $id_pub ?>" class="btn btn-sm btn-outline-secondary" target="_blank">📄 Exporter PDF</a>
        </div>
    </div>
    ...
    <form id="form-commentaire">
        <textarea name="contenu" id="comment-contenu" class="form-control" rows="3" placeholder="Votre commentaire (min 3 caractères)"></textarea>
...
document.querySelectorAll('.like-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                this.querySelector('.like-count').innerText = data.nb_likes;
                if (data.action === 'liked') this.classList.add('liked');
...
document.querySelectorAll('.like-comment-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        let id = this.dataset.id;
        fetch(window.location.pathname + '?action=likeCommentaire', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
document.querySelectorAll('.favori-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                if (data.action === 'added') this.classList.add('active');
...
// Afficher/masquer formulaire de réponse
document.querySelectorAll('.btn-repondre').forEach(btn => {
    btn.addEventListener('click', function() {
        let id = this.dataset.id;
        let formDiv = document.getElementById('reponse-form-' + id);
        formDiv.style.display = formDiv.style.display === 'none' ? 'block' : 'none';
    });
});
document.querySelectorAll('.annuler-reponse').forEach(btn => {
    btn.addEventListener('click', function() {
        let formDiv = this.closest('.reponse-form');
        if (formDiv) formDiv.style.display = 'none';
    });
});

// Soumission réponse (AJAX)
document.querySelectorAll('.form-reponse').forEach(form => {
    form.addEventListener('submit', function(e) {
        e.preventDefault();
        let parentId = this.dataset.parent;
        let textarea = this.querySelector('textarea[name="contenu"]');
...
document.getElementById('form-commentaire')?.addEventListener('submit', function(e) {
    e.preventDefault();
    let contenu = document.getElementById('comment-contenu').value.trim();
    let errDiv = document.getElementById('commentError');
    if (contenu.length < 3) {
        errDiv.innerText = 'Minimum 3 caractères.';
        return;
    }
    errDiv.innerText = '';
