...
$candidatures = $controller->getCandidaturesByUser($id_user)->fetchAll(PDO::FETCH_ASSOC);
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card {
            background: #fff;
            border-radius: 16px;
            box-shadow: 0 4px 24px rgba(67,94,190,.08);
            overflow: hidden;
        }
        .card-header {
            padding: 20px 24px;
        ...
        .btn-supprimer:hover { background: #f7c1c1; }
        ...
        .btn-voir-offres:hover { background: #3348a8; }
        ...
        .modal-box { background:#fff; border-radius:16px; width:100%; max-width:520px; max-height:92vh; overflow-y:auto; box-shadow:0 24px 64px rgba(0,0,0,.25); animation:modalIn .22s ease; }
        ...
        .btn-close-x { background:#f5f6fa; border:none; border-radius:8px; width:32px; height:32px; font-size:18px; cursor:pointer; color:#888; }
        ...
        .m-input { width:100%; border:1px solid #dee2e6; border-radius:8px; padding:10px 14px; font-size:14px; font-family:inherit; }
        .m-input:focus { outline:none; border-color:#435ebe; box-shadow:0 0 0 3px rgba(67,94,190,.12); }
        .btn-save { background:#435ebe; border:none; color:#fff; border-radius:8px; padding:10px 24px; font-size:14px; font-weight:600; cursor:pointer; }
        .btn-save:hover { background:#3348a8; }
        .btn-cancel { background:#f5f6fa; border:none; color:#666; border-radius:8px; padding:10px 20px; font-size:14px; cursor:pointer; }
        .btn-cancel:hover { background:#e8e9ef; }
...
<nav class="navbar">
   <img src="assets/img/logo/logo.png" style="width:200px;">
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <h4>📁 Historique des candidatures</h4>
            <span><?= count($candidatures) ?> candidature<?= count($candidatures) > 1 ? 's' : '' ?></span>
        </div>
        <div style="overflow-x:auto">
            <table>
                ...
                <?php if (count($candidatures) > 0): ?>
                    ...
                    <tr>
                        <td>
                            <div class="offre-title"><?= htmlspecialchars($c['titre_offre'] ?? '—') ?></div>
                            <div class="offre-addr">📍 <?= htmlspecialchars($c['adresse'] ?? '—') ?></div>
                        </td>
                        <td>
                            <?php if (!empty($c['cv'])): ?>
                        ...
                        <td>
                            <div class="lettre-text" title="<?= htmlspecialchars($c['Lettre'] ?? '') ?>">
                                <?= htmlspecialchars($c['Lettre'] ?? '—') ?>
                            </div>
                        </td>
                        <td>
                            <span class="date-badge">📅 <?= htmlspecialchars($c['Date'] ?? '—') ?></span>
                        </td>
                        <td>
                            <?php if ($s === 'en attente'): ?>
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                <a href="offres.php" class="btn-voir-offres">🔍 Voir les offres</a>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
        ...
        <form method="POST" action="modifierCandidature.php" id="editForm"
            ...
            <div class="modal-body">
                <div class="form-group" style="margin-bottom:16px">
                    <label class="field-label">📎 Nouveau CV (laisser vide pour garder l'ancien)</label>
                    <input type="file" class="m-input" name="cv" id="edit_cv" accept=".pdf,.doc,.docx">
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box" style="max-width:440px">
        ...
        <div class="modal-body" style="text-align:center;padding:32px 24px">
            <div style="width:64px;height:64px;background:#ffe5e5;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:28px">🗑️</div>
            <h6 style="font-size:16px;font-weight:700;color:#2d3748;margin-bottom:8px">Êtes-vous sûr ?</h6>
            <p style="color:#6c757d;font-size:14px;margin-bottom:6px">Vous allez retirer votre candidature pour :</p>
            <p id="delete-titre" style="font-weight:700;color:#dc3545;font-size:15px;margin-bottom:6px"></p>
        ...
        <div class="modal-foot" style="justify-content:center;gap:12px">
            ...
            <a id="delete-confirm" href="#"
               style="background:#dc3545;color:#fff;border-radius:8px;padding:10px 28px;font-size:14px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:6px">
                🗑️ Oui, supprimer
            </a>
        </div>
    </div>
