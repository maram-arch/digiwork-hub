...
/* Utilisateur connecté — adapter selon votre système d'auth */
session_start();
...
<head>
    ...
    <style>
        ...
        .btn-close-x {
            background: #f5f6fa; border: none; border-radius: 8px;
        ...
        .m-form-control:focus {
            border-color: #435ebe; outline: none;
        ...
        .btn-send {
            background: #435ebe; border: none; color: #fff; border-radius: 8px;
            padding: 10px 24px; font-size: 14px; font-weight: 600; cursor: pointer;
            display: flex; align-items: center; gap: 6px; transition: background .15s;
        }
        .btn-send:hover { background: #3348a8; }
        .btn-cancel-modal {
            background: #f5f6fa; border: none; color: #666; border-radius: 8px;
            padding: 10px 20px; font-size: 14px; cursor: pointer; transition: background .15s;
        }
        .btn-cancel-modal:hover { background: #e8e9ef; }
        .info-auto {
            background: #e6f1fb; border-radius: 8px;
            padding: 10px 14px; font-size: 12px; color: #185fa5; margin-top: 4px;
        }
        /* ══ Hero ══ */
        .offres-hero {
            background: linear-gradient(135deg, #435ebe 0%, #263587 100%);
            padding: 60px 0 50px; color: #fff; margin-bottom: 40px;
        }
                ...
                <div class="offre-card">
                    ...
                    <div class="offre-meta">
                        <div class="offre-meta-item">
                            <svg xmlns="http://www.w3.org/20/svg" width="13" height="13"
                                 viewBox="0 0 24 24" fill="none" stroke="#6c757d"
                    ...
                    <div class="offre-actions">
                        <a href="detail_offre.php?id=<?= urlencode($offre['id_offer']) ?>"
                           class="btn-detail">
                            <svg xmlns="http://www.w3.org/20/svg" width="13" height="13"
                        ...
                        <button class="btn-postuler"
                            ...
                            <svg xmlns="http://www.w3.org/20/svg" width="14" height="14"
                                 viewBox="0 0 24 24" fill="none" stroke="currentColor"
...
<div class="modal-overlay" id="postulerModal">
    <div class="modal-box">
        <div class="modal-top">
            <h5>
                <div class="modal-icon-wrap">
                    <svg xmlns="http://www.w3.org/20/svg" width="17" height="17"
                         viewBox="0 0 24 24" fill="none" stroke="#435ebe"
                         stroke-width="2.2" stroke-linecap="round">
                        <line x1="22" y1="2" x2="11" y2="13"/>
                        <polygon points="22 2 15 22 11 13 2 9 22 2"/>
                    </svg>
        ...
        <form method="POST" action="postuler.php" id="postulerForm"
            ...
            <input type="hidden" name="id_offre" id="modal_id_offre">
            ...
            <div class="modal-body-inner">
                ...
                <div class="mb-3">
                    ...
                    <small style="color:#6c757d;font-size:12px" id="lettre_count">0 / 20 caractères</small>
                    <div class="error-message"></div>
                </div>
            ...
            <div class="modal-foot">
                <button type="button" class="btn-cancel-modal"
                        onclick="closePostulerModal()">Annuler</button>
                <button type="submit" class="btn-send">
                    <svg xmlns="http://www.w3.org/20/svg" width="15" height="15"
                         viewBox="0 0 24 24" fill="none" stroke="currentColor"
                         stroke-width="2.2" stroke-linecap="round">
                        <line x1="22" y1="2" x2="11" y2="13"/>
                        <polygon points="22 2 15 22 11 13 2 9 22 2"/>
...
function openPostulerModal(idOffre, titreOffre) {
    ...
    document.getElementById('lettre_count').textContent = '0 / 20 caractères';
...
document.getElementById('input_lettre').addEventListener('input', function() {
    var len = Math.min(this.value.length, 20);
    document.getElementById('lettre_count').textContent = len + ' / 20 caractères';
...
function validatePostulerForm(event) {
    ...
    var lettreInput = document.getElementById('input_lettre');
    var lettre     = lettreInput.value.trim();
    var valid      = true;
 
    /* CV */
    if (!cvInput.files || cvInput.files.length === 0) {
        setError(cvInput, 'Veuillez joindre votre CV');
        valid = false;
    } else {
        ...
        if (!allowed.includes(file.type)) {
            setError(cvInput, 'Format accepté : PDF, DOC, DOCX uniquement');
            valid = false;
        } else if (file.size > 2 * 1024 * 1024) {
            setError(cvInput, 'Le fichier ne doit pas dépasser 2 Mo');
            valid = false;
        }
    }
    ...
    /* Lettre */
    if (!lettre) {
        setError(lettreInput, 'La lettre de motivation est obligatoire');
        valid = false;
    } else if (lettre.length < 10) {
        setError(lettreInput, 'Minimum 10 caractères requis');
        valid = false;
    }
